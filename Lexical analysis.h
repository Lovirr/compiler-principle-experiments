#ifndef CPLUS_LIBRARY_LEXICAL_ANALYSIS_H
#define CPLUS_LIBRARY_LEXICAL_ANALYSIS_H
...
void loadProperty() {
    ...
    if (!fin.is_open()) {
        cout << "failed to find ketword.txt" << endl;
        return;
    }
    property p;
    while (!fin.eof()) {
...
void Finput() {
    ...
    fin.close();
    cout << "The text of the C program is:" << endl;
    cout << code << endl;
    // 对程序进行词法分析
    Test(code);
}
...
#endif //CPLUS_LIBRARY_LEXICAL_ANALYSIS_H
